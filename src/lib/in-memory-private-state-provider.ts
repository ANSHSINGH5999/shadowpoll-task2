// Adapted from midnightntwrk/example-bboard (bboard-ui/src/in-memory-private-state-provider.ts)
// Copyright (C) Midnight Foundation — SPDX-License-Identifier: Apache-2.0
//
// A minimal in-memory PrivateStateProvider. Private state (the voting credential)
// lives only in this tab's memory and is lost on refresh — fine for this
// demo. For persistence across sessions, swap this for
// @midnight-ntwrk/midnight-js-level-private-state-provider's
// `levelPrivateStateProvider`, which stores encrypted state in the browser.
...
> => {
  ...
  const exportPrivateStatePayload = (address: ContractAddress): Record<string, string> =>
    Object.fromEntries(
      Array.from(getScopedStates(address).entries()).map(([stateId, value]) => [stateId as string, encode(value)]),
    );
